...
static void GenerateNew(sf::RenderWindow &window, double &t, double *params) {
  ...
  const std::string equation_str = "x' = " + MakeEquationStr(params) +
                                   "\n"
                                   "y' = " +
                                   MakeEquationStr(params + num_params / 2) +
                                   "\n"
                                   "Code: " +
                                   equ_code;
...
static void CenterPlot(const std::vector<sf::Vector2f> &history) {
  ...
  plot_scale =
      1.0f / std::max(std::max(max_x - min_x, max_y - min_y) * 0.6f, 0.1f);
}
...
int main(int argc, char *argv[]) {
  ...
  std::cout << std::endl;
  std::cout << "    These are plots of random recursive equations, which"
            << std::endl;
  std::cout << "often produce chaos, and results in beautiful patterns."
            << std::endl;
  std::cout << "For every time t, a point (x,y) is initialized to (t,t)."
            << std::endl;
  std::cout << "The equation is applied to the point many times, and each"
            << std::endl;
    ...
    while (window.isOpen()) {
      ...
      for (int step = 0; step < steps; ++step) {
        ...
        for (int iter = 0; iter < iters; ++iter) {
          ...
          if (screenPt.x > 0.0f && screenPt.y > 0.0f && screenPt.x < window_w &&
              screenPt.y < window_h) {
            ...
            rolling_delta =
                std::min(rolling_delta, std::max(delta / (dist + 1e-5),
      ...
      window.draw(vertex_array.data(), vertex_array.size(),
